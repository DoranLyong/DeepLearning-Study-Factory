...
include more batch_sizes you want to try and more learning rates!
"""
...
#        cudnn.benchmark = False   
        ...
        """
        DataParallel 을 사용하니 writer.add_histogram('fc1', model.fc1.weight)  부분에서 에러남. 
        모델을 쪼개서 로드하기 때문에 어디서 불러올지 몰라서 그런건가? 
        """
        ...
        optimizer = optim.Adam( model.parameters(), lr= lr, weight_decay=0.0)  # 네트워크의 모든 파라미터를 전달한다 



        """ 텐서보드 객체 초기화 
        """
        writer = SummaryWriter(f'runs/MNIST/MiniBatchSize {batch_size} LR {lr}') # 학습 로그(log)가 저장되는 경로 초기화 
        ...
        for epoch in range(0, num_epochs):
            ...
            for batch_idx, (data, targets) in enumerate(train_loader): # 미니배치 별로 iteration 
                ...
                img_grid = torchvision.utils.make_grid(data)   # 현재 로드된 이미지 데이터를 grid 형태로 만들어서 보여지기 쉽게 만듬 
            ...
            writer.add_hparams({'lr': lr, 'bsize': batch_size}, 
                                {'accuracy': float(num_correct)/float(num_samples)*100, # epoch당 총 학습 정확도
                                'loss': sum(train_losses)/len(train_losses)}   # epoch당 총 평균 학습 손실 
                                )
